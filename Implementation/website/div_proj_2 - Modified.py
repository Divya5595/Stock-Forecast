...
##    end_date = input("Enter prediction date(YYYY-MM-DD):")
    end_date = datetime.datetime(2017,4,23)
    start_date = end_date
    print (end_date)
    return
...
def Actual_Value():
    ...
    df = data.DataReader(stockTicker, 'yahoo', '2017-04-21', '2017-04-23')
    ao=df['Close']
    print (str(ao))
    return
...
start_date += datetime.timedelta(weeks=-120)
...
with open('ltcsv.csv', "w", newline="") as f:
    w = csv.writer(f)
